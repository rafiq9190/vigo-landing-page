import React from "react"

function WorkingWithUs() {
  return (
    <>
      <div className="container mx-auto px-3 text-center working w-3/6 ">
        <p className="hero-p1  lg: mt-16 text-rock-500 text-5xl font-medium  ">
          Increase your appearing at the top of the major search engines
        </p>
        <p className="text-xl text-lynch-500 mt-4 mb-4">
          The stats are in our favour. Vigorant doesn't just help you with your
          marketing, we get you customer relationships that last.
        </p>
      </div>
      <div className="lg:mx-20 px-6 ">
        <div className="block    lg:bg-panache-500 border-panache-500 border-2 flex-wrap border rounded-[24px] flex   justify-around mt-2 bt-2 	">
          <div className=" w-screen lg:w-1/3 my-8 text-center px-20">
            <p className="text-plam-500 text-3xl font-semibold">+153%</p>
            <p className="text-center text-viridian-500 text-base px-10 mt-2  ">
              Increased Brand Awareness
            </p>
          </div>
          <div className=" w-screen lg:w-1/3  my-8 text-center px-10 ">
            <p className="text-plam-500 text-3xl font-semibold">+98%</p>
            <p className="text-viridian-500  text-center text-base px-20 mt-2">
              Increased Website visitors
            </p>
          </div>
          <div className=" w-screen lg:w-1/3   my-8 text-center px-10">
            <p className="text-plam-500 text-3xl font-semibold">+175%</p>
            <p className="text-viridian-500 text-base px-20 mt-2">
              Increased Conversation Rates
            </p>
          </div>
        </div>
      </div>
    </>
  )
}

export default WorkingWithUs
